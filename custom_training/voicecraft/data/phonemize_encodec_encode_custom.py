...
print(f"{espeak_ng_path}")
# These are needed for VoiceCraft
os.environ["USER"] = getpass.getuser()
os.environ['PHONEMIZER_ESPEAK_PATH'] = os.path.join(os.path.dirname(espeak_ng_path), "espeak-ng.exe")
os.environ['PHONEMIZER_ESPEAK_LIBRARY'] = os.path.join(os.path.dirname(espeak_ng_path), "libespeak-ng.dll")
...
def parse_args():
    ...
    parser.add_argument('--download_to', type=str, default=f"C:/AI/VoiceCraft/datasets/{character}_debug", help="dir where you want the huggingface gigaspeech dataset to be downloaded to")
    parser.add_argument('--save_dir', type=str, default=f"C:/AI/VoiceCraft/datasets/{character}_phn_enc_manifest_debug", help="path to the manifest, phonemes, and encodec codes dirs")
    parser.add_argument('--encodec_model_path', type=str, default="C:/AI/VoiceCraft/encodec_4cb2048_giga.th")
...
if __name__ == "__main__":
    ...
    gs = datasets.load_dataset("custom_generator.py", name="custom_audio_dataset", data_dir=dataset_folder, num_proc=8)
    logging.info(f"time spend on loading the dataset: {time.time() - stime:.2f} seconds")

    splits = ['validation', 'train']
    
    logging.info(f"custom dataset {args.dataset_size} info: {gs}")
    ...
    splits = ['validation', 'train']
    loaders = [validation_loader, train_loader]
        ...
        for m, mega_batch in enumerate(loader):
            ...
            for n in tqdm.tqdm(range(n_steps), disable=True):
                ...
                for i, length in enumerate(all_lens):
                    ...
                    #print(f" Saving Codec: {segment_id_batch[i]} {actual_len} original length {length}")
                    manifest_line = f"0\t{segment_id_batch[i]}\t{actual_len}"
                    manifest_lines.append(manifest_line)

    manifest_directory = os.path.join(args.save_dir, args.dataset_size, "manifest")
    os.makedirs(manifest_directory, exist_ok=True)

    random.shuffle(manifest_lines)

    # Calculate the split point
    split_point = int(0.7 * len(manifest_lines))

    # Split the array into training and validation sets
    train = manifest_lines[:split_point]
    val = manifest_lines[split_point:]

    # Write the train manifest to a file
    train_manifest_path = os.path.join(manifest_directory, "train.txt")
    with open(train_manifest_path, 'w', encoding='utf-8') as train_file:
        for line in train:
            train_file.write(line + "\n")

    validation_manifest_path = os.path.join(manifest_directory, "validation.txt")
    with open(validation_manifest_path, 'w', encoding='utf-8') as validation_file:
        for line in val:
            validation_file.write(line + "\n")

    print(f"Train manifest created at {train_manifest_path}")
    print(f"Validation manifest created at {validation_manifest_path}")
