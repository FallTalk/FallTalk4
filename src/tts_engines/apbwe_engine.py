...
class APBWEUpscaler:
    def __init__(self, checkpoint_path: str, device: str = "cuda"):
        ...
        # Load model
        self.model = APNet_BWE_Model(self.h).to(self.device).eval()
    ...
    def upscale(self, audio_np: np.ndarray, sr: int = 16000) -> np.ndarray:
        if audio_np.ndim > 1:
            audio_np = np.mean(audio_np, axis=0)

        print(f"[INFO] Input audio: {len(audio_np) / sr:.2f}s @ {sr}Hz")

        audio_tensor = torch.from_numpy(audio_np).float().unsqueeze(0).to(self.device)

        audio_hr = aF.resample(audio_tensor, orig_freq=sr, new_freq=self.h.hr_sampling_rate)
        audio_lr = aF.resample(audio_tensor, orig_freq=sr, new_freq=self.h.lr_sampling_rate)
        audio_lr = aF.resample(audio_lr, orig_freq=self.h.lr_sampling_rate, new_freq=self.h.hr_sampling_rate)
        audio_lr = audio_lr[:, :audio_hr.size(1)]

        with torch.no_grad():
            amp_nb, pha_nb, _ = amp_pha_stft(audio_lr, self.h.n_fft, self.h.hop_size, self.h.win_size)
            amp_wb_g, pha_wb_g, _ = self.model(amp_nb, pha_nb)
            audio_hr_g = amp_pha_istft(amp_wb_g, pha_wb_g, self.h.n_fft, self.h.hop_size, self.h.win_size)

        upscaled = audio_hr_g.squeeze().cpu().numpy()
        print(f"[INFO] Output audio: {len(upscaled) / self.h.hr_sampling_rate:.2f}s @ {self.h.hr_sampling_rate}Hz")
        return upscaled, self.h.hr_sampling_rate

class APBWE_SR:
    def __init__(self):
        self.model_24000 = APBWEUpscaler(os.path.abspath(os.path.join(get_app_root(), 'models', 'APBWE', '24kto48k', 'g_24kto48k.zip')), "cpu")
        self.model_16000 = APBWEUpscaler(os.path.abspath(os.path.join(get_app_root(), 'models', 'APBWE', '16kto48k', 'g_16kto48k.zip')), "cpu")
        self.device = cfg.get(cfg.device)

    def upscale(self, audio_np: np.ndarray, sr: int = 16000):
        if sr == 24000:
            self.model_24000.move(self.device)
            us_audio_np, us_sr =  self.model_24000.upscale(audio_np, sr)
            self.model_24000.move("cpu")
            return us_audio_np, us_sr
        elif sr == 16000:
            self.model_16000.move(self.device)
            us_audio_np, us_sr = self.model_16000.upscale(audio_np, sr)
            self.model_16000.move("cpu")
            return us_audio_np, us_sr
        else:
            return audio_np, sr
